...
Strategy A — same `font-family` stack, Latin-first fallback. Each typographic token already lists `"Jost, Noto Sans SC, system-ui, sans-serif"` (or the Lora equivalent). Latin glyphs render in Jost / Lora; CJK glyphs automatically fall through to Noto Sans SC / Noto Serif SC. No per-language class needed. Mixed sentences like `使用 OpenHands 思考` render in one logical run with the correct face per script.
...
- Pangu spacing 盘古之白 (space between CJK and Latin: `使用 OpenHands` not `使用OpenHands`)
