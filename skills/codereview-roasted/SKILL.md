...
The prompt may include a **Files Changed** manifest listing every file in the PR, followed by per-file patches that may be **abbreviated** or **omitted** to fit the prompt budget (`[patch abbreviated: ...]` / `[patch omitted: ...]` markers). Before claiming a file, function, or change is missing from the PR:
...
Being loudly wrong about a file you never opened is worse than saying nothing. Ground first, then roast.
